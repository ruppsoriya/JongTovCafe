  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      {items.map((item, index) => (
        <div
          key={index}
          className="rounded-2xl border border-white/60 bg-white/85 p-4 shadow-sm dark:border-white/10 dark:bg-white/5"
        >
          <div className="text-sm font-semibold text-[#221815] dark:text-white">
            {item}
          </div>
...
export default function CafeDetail({ cafe }) {
      ...
      try {
        ...
        setSimilarCafes(
          (Array.isArray(data) ? data : [])
            .filter((item) => String(item.id) !== String(cafe.id || cafe._id))
            .slice(0, 8),
        );
      } catch (error) {
        setSimilarCafes([]);
  ...
  if (!cafe) {
    return (
      <div className="min-h-screen bg-cream p-8 text-coffee">Loading...</div>
    );
  }
  ...
  const images = (Array.isArray(cafe.images) ? cafe.images : [])
    .map((image) =>
      resolveCafeImageSource(
        { images: [image], name: cafe.name, location: cafe.location },
        API_BASE,
      ),
    )
    .filter(Boolean)
  ...
  const openingEntries =
    cafe.openingHours && typeof cafe.openingHours === "object"
      ? Object.entries(cafe.openingHours)
      : [];
  ...
  return (
    <div className="min-h-screen text-[#231913] dark:text-[#f7ede5]">
      <Head>
        ...
        <meta
          name="description"
          content={cafe.description || `${cafe.name} cafe details`}
        />
      </Head>
      <Navbar />
                ...
                <div className="absolute bottom-5 left-5 right-5 flex items-end justify-between gap-4 text-white">
                  ...
                  <div className="hidden rounded-2xl bg-white/15 px-4 py-3 text-right backdrop-blur md:block">
                    ...
                    <div className="mt-2 text-xs text-white/70">
                      {cafe.location?.address || "Phnom Penh"}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <aside className="space-y-5 rounded-[2rem] border border-white/60 bg-white/85 p-6 shadow-[0_20px_60px_rgba(74,48,33,0.10)] dark:border-white/10 dark:bg-[#1a120f]/90">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="mt-2 text-2xl font-bold text-[#221815] dark:text-white">
                    {cafe.name}
                  </div>
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {(cafe.tags || []).slice(0, 6).map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-900 dark:bg-white/10 dark:text-amber-100"
                  >
                    {tag}
                  </span>
                ))}
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {[
                  {
                    label: "Location",
                    value: cafe.location?.address || "Phnom Penh",
                  },
                  {
                    label: "WiFi speed",
                    value: cafe.wifiSpeed ? `${cafe.wifiSpeed}+` : "Unknown",
                  },
                  { label: "Popularity", value: cafe.popularity || 0 },
                  {
                    label: "Open status",
                    value: cafe.isOpen !== false ? "Open" : "Closed",
                  },
                ].map((item) => (
                  <div
                    key={item.label}
                    className="rounded-2xl bg-[#faf4ee] p-4 dark:bg-white/5"
                  >
                    <div className="text-xs uppercase tracking-[0.18em] text-gray-500 dark:text-gray-400">
                      {item.label}
                    </div>
                    <div className="mt-2 text-sm font-semibold text-[#221815] dark:text-white">
                      {item.value}
                    </div>
                  </div>
                ))}
              ...
              <div className="flex flex-wrap gap-3">
                <button
                  onClick={addFavorite}
                  className="rounded-2xl bg-[#2f221d] px-4 py-3 text-sm font-semibold text-white transition hover:translate-y-[-1px]"
                >
                  Save to favorites
                </button>
              ...
              {message && (
                <p className="rounded-2xl bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:bg-white/5 dark:text-amber-100">
                  {message}
                </p>
              )}
            </aside>
        ...
        <section className="container mx-auto grid gap-6 px-4 pb-10 scroll-mt-28 lg:grid-cols-[1fr_0.75fr]">
          <div className="space-y-6">
            <div className="rounded-[2rem] border border-white/60 bg-white/85 p-6 shadow-[0_20px_60px_rgba(74,48,33,0.10)] dark:border-white/10 dark:bg-[#1a120f]/90">
              ...
              <p className="mt-4 text-sm leading-7 text-gray-700 dark:text-gray-300">
                {cafe.description ||
                  "A modern cafe tailored for coffee lovers, students, and remote workers."}
              </p>

              <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {(cafe.facilities || ["WiFi", "Seating", "Coffee"]).map(
                  (facility) => (
                    <div
                      key={facility}
                      className="rounded-2xl bg-[#faf4ee] px-4 py-3 text-sm font-medium text-[#221815] dark:bg-white/5 dark:text-white"
                    >
                      {facility}
                    </div>
                  ),
            ...
            <div className="rounded-[2rem] border border-white/60 bg-white/85 p-6 shadow-[0_20px_60px_rgba(74,48,33,0.10)] dark:border-white/10 dark:bg-[#1a120f]/90">
              ...
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                A lightweight preview based on cafe tags and available
                facilities.
              </p>
              <div className="mt-5">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                {openingEntries.length ? (
                  openingEntries.map(([day, hours]) => (
                    <div
                      key={day}
                      className="rounded-2xl bg-[#faf4ee] px-4 py-3 text-sm text-[#221815] dark:bg-white/5 dark:text-white"
                    >
            ...
            <div className="rounded-[2rem] border border-white/60 bg-white/85 p-6 shadow-[0_20px_60px_rgba(74,48,33,0.10)] dark:border-white/10 dark:bg-[#1a120f]/90">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">
                    Ratings and feedback from the community.
                  </p>
                </div>
              ...
              <form
                ...
                className="mt-6 rounded-[1.5rem] border border-amber-100 bg-amber-50/60 p-5 dark:border-white/10 dark:bg-white/5"
              >
                <div className="grid gap-4 md:grid-cols-[160px_1fr]">
                  <div>
                    <label className="mb-2 block text-sm font-semibold text-[#221815] dark:text-white">
                      Your rating
                    </label>
                    <select
              ...
              <div className="mt-6 space-y-4">
                {reviews.map((review) => (
                  <div
                    key={review.id}
                    className="rounded-[1.5rem] border border-white/60 bg-white/85 p-5 dark:border-white/10 dark:bg-white/5"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
          ...
          <div className="space-y-6">
            <div className="rounded-[2rem] border border-white/60 bg-white/85 p-6 shadow-[0_20px_60px_rgba(74,48,33,0.10)] dark:border-white/10 dark:bg-[#1a120f]/90">
              ...
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                Open the exact spot on Google Maps or use the embedded location
                below.
              </p>
              <iframe
                title="Cafe Map"
                src={mapSrc}
                className="mt-5 h-[420px] w-full rounded-[1.5rem] border-0"
                loading="lazy"
              />
            </div>
            ...
            <div className="rounded-[2rem] border border-white/60 bg-white/85 p-6 shadow-[0_20px_60px_rgba(74,48,33,0.10)] dark:border-white/10 dark:bg-[#1a120f]/90">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">
                    Recommended based on this cafe’s vibe.
                  </p>
                </div>
                {similarLoading && (
            ...
            <div className="rounded-[2rem] border border-white/60 bg-white/85 p-6 shadow-[0_20px_60px_rgba(74,48,33,0.10)] dark:border-white/10 dark:bg-[#1a120f]/90">
              ...
              <div className="mt-4 space-y-3 text-sm">
                <button
                  onClick={addFavorite}
                  className="w-full rounded-2xl bg-[#2f221d] px-4 py-3 font-semibold text-white transition hover:translate-y-[-1px]"
                >
                  Save cafe
                </button>
                <Link
                  href="/"
                  className="block rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-center font-semibold text-amber-900 dark:border-white/10 dark:bg-white/5 dark:text-amber-100"
                >
                  Back to home
                </Link>
              </div>
