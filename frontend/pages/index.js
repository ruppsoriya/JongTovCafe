      ...
      <main id="main-content" className="pt-24">
          ...
          <div className="container mx-auto grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
            <div className="max-w-3xl">
              ...
              <p className="mt-6 max-w-2xl text-lg leading-8 text-gray-700 dark:text-gray-300">
                Search by coffee quality, WiFi, quiet corners, price, and atmosphere. Jong Tov Cafe turns Phnom Penh’s coffee scene into a recommendation experience that feels modern and personal.
              </p>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                {quickFilterPresets.map((preset) => (
                  <button
                    ...
                    className={`rounded-full border px-4 py-2 text-sm font-medium transition ${filters[preset.key]
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-3">
                {[
                  ...
                  <div key={item.label} className="rounded-3xl border border-white/70 bg-white/80 p-5 shadow-[0_12px_40px_rgba(74,48,33,0.08)] dark:border-white/10 dark:bg-white/5">
                    <div className="text-sm text-gray-500 dark:text-gray-300">{item.label}</div>
                    <div className="mt-2 text-3xl font-black tracking-tight text-[#221815] dark:text-white">{item.value}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid gap-4 rounded-[2rem] border border-white/60 bg-white/75 p-5 shadow-[0_24px_80px_rgba(74,48,33,0.12)] backdrop-blur-xl dark:border-white/10 dark:bg-[#1a120f]/85">
              <div className="rounded-[1.5rem] bg-gradient-to-br from-[#3b2f2f] via-[#5f4031] to-[#e07a5f] p-6 text-white shadow-lg">
                <div className="text-xs font-semibold uppercase tracking-[0.28em] text-white/75">Recommendation engine</div>
                <h2 className="mt-4 text-2xl font-bold">Cafe picks that match your preferences.</h2>
                <p className="mt-3 max-w-md text-sm leading-6 text-white/80">
                  Your selected filters shape content-based and popularity scoring so the list adapts to work mode, study mode, or a relaxed weekend search.
                </p>
              </div>

              <FiltersSidebar filters={filters} setFilters={setFilters} onApply={() => runSearch(filters, query)} onReset={handleReset} />
            </div>
          </div>
        </section>
        ...
        <section className="container mx-auto px-4 pb-12 scroll-mt-28">
          <div className="mb-6 flex items-end justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-amber-700 dark:text-amber-200">Recommendations</p>
              <h2 id="recommendations" className="mt-2 text-3xl font-bold tracking-tight text-[#221815] dark:text-white">
                Cafes tailored to your vibe
              </h2>
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-300">Powered by your filters, favorites, and popularity signals</p>
          </div>
        ...
        <section className="container mx-auto px-4 py-8 scroll-mt-28">
          <div className="mb-6 flex items-end justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-amber-700 dark:text-amber-200">Featured</p>
              <h2 id="featured" className="mt-2 text-3xl font-bold tracking-tight text-[#221815] dark:text-white">
                Featured cafes
              </h2>
            </div>
            <button type="button" onClick={() => runSearch(filters, query)} className="text-sm font-semibold text-amber-700 transition hover:text-amber-900 dark:text-amber-200">
              Refresh results
            </button>
          </div>
        ...
        <section className="container mx-auto px-4 py-8">
          <div className="mb-6">
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-amber-700 dark:text-amber-200">Popular</p>
            <h2 className="mt-2 text-3xl font-bold tracking-tight text-[#221815] dark:text-white">Popular cafes carousel</h2>
          </div>
          <div className="flex gap-5 overflow-x-auto pb-4 snap-x snap-mandatory">
            {popularCafes.map((cafe) => (
              <div key={cafe.id || cafe._id} className="min-w-[320px] max-w-[320px] snap-start">
                <CafeCard cafe={cafe} />
              </div>
            ))}
          </div>
