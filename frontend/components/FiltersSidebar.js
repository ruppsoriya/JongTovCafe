export default function FiltersSidebar({ filters, setFilters, onApply, onReset }) {
  ...
  return (
    <aside className="rounded-3xl border border-white/60 bg-white/85 p-5 shadow-[0_20px_60px_rgba(74,48,33,0.08)] backdrop-blur-xl dark:border-white/10 dark:bg-[#1b1410]/80 dark:text-white lg:sticky lg:top-28 lg:max-h-[calc(100vh-7rem)] lg:overflow-auto">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-amber-600 dark:text-amber-300">Filters</p>
          <h4 className="text-lg font-semibold">Refine your search</h4>
        </div>
        <button type="button" onClick={onReset} className="text-sm font-medium text-amber-700 transition hover:text-amber-900 dark:text-amber-300">
          Reset
        </button>
      </div>
      ...
      <div className="space-y-4">
        <div>
          <label className="mb-2 block text-sm font-medium text-gray-600 dark:text-gray-300">Minimum rating</label>
          <select
        ...
        <div>
          <label className="mb-2 block text-sm font-medium text-gray-600 dark:text-gray-300">Price range</label>
          <select
            className="w-full rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-amber-400 dark:border-white/10 dark:bg-white/5 dark:text-white"
