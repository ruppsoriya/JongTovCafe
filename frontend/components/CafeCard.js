...
export default function CafeCard({ cafe }) {
  ...
  const wifiIcon =
    cafe.wifiSpeed >= 70 ? "📶" : cafe.wifiSpeed > 0 ? "📡" : "☕";
  ...
  const tags = (
    cafe.highlights && cafe.highlights.length
      ? cafe.highlights
      : cafe.tags || []
  ).slice(0, 3);
  ...
  return (
    ...
    >
      <article className="flex h-full flex-col overflow-hidden rounded-3xl border border-white/70 bg-white/90 shadow-[0_18px_50px_rgba(74,48,33,0.10)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_28px_70px_rgba(74,48,33,0.18)] group-focus-visible:ring-2 group-focus-visible:ring-amber-300 dark:border-white/10 dark:bg-[#1b1410]/90">
        <div
          className={`relative h-56 overflow-hidden bg-gradient-to-br ${bgGradient}`}
        >
          {imageSrc ? (
            <img
              src={imageSrc}
          ...
          <div className="absolute left-4 top-4 flex gap-2">
            <span
              className={`rounded-full px-3 py-1 text-xs font-semibold ${isOpen ? "bg-emerald-500/90 text-white" : "bg-white/90 text-gray-900"}`}
            >
        ...
        <div className="flex flex-1 flex-col gap-4 p-5">
          <div>
            <div className="mb-2 flex items-start justify-between gap-4">
              <div>
                <h3 className="text-xl font-bold tracking-tight text-[#221815] dark:text-white">
                  {nameValue}
                </h3>
            ...
            <p className="line-clamp-2 text-sm leading-6 text-gray-600 dark:text-gray-300">
              {cafe.description ||
                "A coffee stop curated for study sessions, remote work, or easy meetups."}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-sm text-gray-600 dark:text-gray-300">
            <div className="rounded-2xl bg-[#faf4ee] px-3 py-2 dark:bg-white/5">
              {wifiIcon} WiFi {cafe.wifiSpeed || 0}
            </div>
            <div className="rounded-2xl bg-[#faf4ee] px-3 py-2 dark:bg-white/5">
          ...
          <div className="mt-auto flex flex-wrap gap-2">
            {facilities.map((facility) => (
              <span
                key={facility}
                className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-medium text-amber-900 dark:border-white/10 dark:bg-white/5 dark:text-amber-100"
              >
                {facility}
              </span>
            ))}
