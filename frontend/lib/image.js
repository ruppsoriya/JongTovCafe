export function resolveCafeImage(image) {
  ...
  if (
    /\.(jpeg|jpg|png|gif|webp|svg|bmp)(?:\?|$)/i.test(raw) &&
  ...
  const extractDirect = (value) => {
    const directMatch = value.match(
      /https?:\/\/lh[35]\.googleusercontent\.com[^\s"'<>)]+/i,
    );
    return directMatch ? directMatch[0] : null;
  };

  if (
    raw.includes("lh3.googleusercontent.com") ||
    raw.includes("lh5.googleusercontent.com")
  ) {
